(ns filmprodops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (FilmProductionAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the FilmProductionGovernor (:govern)
  ...
  human operator/production coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [filmprodops.advisor :as advisor]
            [filmprodops.governor :as governor]
...
(defn build
  ...
  `filmprodops.store/Store`).
  opts:
    :advisor      -- a `filmprodops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FilmProductionAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; FilmProductionGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :production-id (:production-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :production-id (:production-id request) :by (:by approval)}]}
            {:disposition :hold
